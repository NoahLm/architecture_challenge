...
# Import custom functions for model loading and predictions
...
#Loading the Investment Balance Prediction Model from S3
saldos_inversion_model = load_saldos_inversion_model()

#Note: For purpouses of this project the model has been loaded to streamlit
#However it is better to connect the webapp to a AWS Sagmaker endpoint to scale resources if necessary

def main():
    #Webpage title and sidebar with query options
    ...
    if option == "Saldos promedio":
        ...
        # Checks if any column has been selected before running the query
        if columns:
    ...
    # The process repeats in all the other options
        ...
        # Prepares data for Saldo de Inversion prediction
